...
CBigNum bnSTOPPOW(~uint256(0) >> 205);

CBigNum bnProofOfWorkLimitTestNet(~uint256(0) >> 16);

unsigned int nStakeMinAge = 60 * 60 * 24 * 1; // 24 hours as zero time weight
unsigned int nStakeMaxAge = 60 * 60 * 24 * 14; // 14 days as full weight
...
const string strMessageMagic = "EiniaOraCoin Signed Message:\n";
...
int64_t CTransaction::GetMinFee(unsigned int nBlockSize, bool fAllowFree, enum GetMinFee_mode mode, unsigned int nBytes) const
{
    ...
    if(IsCoinStake())
    {
        ...
        nMinTxFee = 0.0001 * COIN;
        nMinRelayTxFee = 0.0001 * COIN;
    }
...
int64_t GetProofOfWorkReward(unsigned int nBits, int nHeight)
{
	if (nHeight == 1)
		return 31755 * COIN;
	else if (nHeight < 101)
...
{
    int64_t nRewardCoinYear, nSubsidy = 0;
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("EiniaOraCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in EiniaOraCoin
    ...
    if (IsProofOfWork())
    {
        int64_t nBlockReward = GetProofOfWorkReward(nBits, pindex->nHeight);
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // EiniaOraCoin: check proof-of-stake block signature
...
uint256 CBlockIndex::GetBlockTrust() const
{
    ...
    // Return nPoWTrust for the first 50 blocks, or after PoW is over
    if (pprev == NULL || pprev->nHeight < 50 || pprev->nHeight > 6000)
        return nPoWTrust;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "EiniaOraCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "EiniaOraCoin";
        CTransaction txNew;
        txNew.nTime = 1432656588;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x02c3265f571fa924f2cc077acd43ade6bf370dbc0d55390e96e2a46fd6ffadfd"));
        block.print();
