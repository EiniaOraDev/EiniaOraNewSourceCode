...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfWorkReward(nBits,GetLastBlockIndex(pindexBest, false)->nHeight);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue",    (uint64_t)GetProofOfWorkReward(GetLastBlockIndex(pindexBest, false)->nBits, (GetLastBlockIndex(pindexBest, false)->nHeight)+1)));
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "EiniaOraCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "EiniaOraCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "EiniaOraCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "EiniaOraCoin is downloading blocks...");
